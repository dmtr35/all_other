...
class UserService {
    async registration(email, password) {                                               // асинхронная функция для регистрации пользователя. параметры принимает email, и password(который мы будем получать в теле запроса)
        const candidat = await UserModel.findOne({ email })                               // убеждаемся что в базе данных с таким email нет записи, findOne осуществит поиск по полю email
        if (candidat) {                                                                 // делаем проверку, если candidat не равен null(в нем что то находится), то мы пробрасываем ошибку
        ...
        const activationLink = uuid.v4()          // нужно указать ссылку по которой пользователь будет активировать аккаунт и подтверждать что email принадлежит ему // uuid.v4() вернет нам рандомную уникальную строку
