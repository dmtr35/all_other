...
const UserSchema = new Schema({                                  // создаем схему, схема описывает какие поля будет содержать сущьность пользователя.  Передаем обьект и какие поля будут у пользователя
...
module.exports = model('User', UserSchema)                       // из этого файла экспортируем модель которую создаем на основании схемы которую мы сделали. (первый параметр название модели, а вторым передаем схему)
