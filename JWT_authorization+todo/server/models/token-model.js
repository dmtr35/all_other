...
const TokenSchema = new mongoose.Schema({                                  // создаем схему, схема для хранения refreshToken и id пользователя (сюда можно добавить ip-адрес с которого зашел пользователь)
    user: {type: mongoose.Schema.Types.ObjectId, ref: 'User'},             // ссылка на пользователя, тип: ObjectId который достаем из схемы поле Types. это поле будет ссылаться на модель пользователя(ref: 'User')
